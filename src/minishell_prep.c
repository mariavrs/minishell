...
/*   Updated: 2023/04/09 07:04:05 by mvorslov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	msh_prep(t_msh *msh, char **envp)
{
	...
	msh->ex_sline = NULL;
	msh->cmd_list = NULL;
/* 	msh->stdin_backup = dup(STDIN_FILENO);
	msh->stdout_backup = dup(STDOUT_FILENO);
	if (msh->stdin_backup == -1 || msh->stdout_backup == -1)
		return (ft_free_exit(msh), 1); */
